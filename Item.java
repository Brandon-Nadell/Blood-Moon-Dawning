...
public class Item extends Entity implements Chestable, Cloneable {
	...
	public enum Items {
		UNKNOWN("item/unknown.png", Color.WHITE), PAPER("item/paper.png", Color.WHITE),
		BONE("item/bone.png", Color.WHITE, Essences.UNDEAD), GUTS("item/guts.png", Color.DARK_GRAY, Essences.UNDEAD),

		PLASMA_BOX("item/gear/plasma_box.png", Color.GOLD), SKULL("item/weapon/special/skull.png", Color.LIGHT_GRAY),
		HOLY_WATER("item/gear/holy_water.png", Color.ROYAL), FLY_TRAP("item/gear/fly_trap.png", Color.GREEN),
		...
		WRATH_DASHER("item/gear/wrath_dasher.png", Color.FIREBRICK), PILLOW("item/gear/pillow.png", Color.LIGHT_GRAY),
		INFINITY_POTION("item/gear/infinity_potion.png", Color.PINK), EMERALD("item/gear/emerald.png", Color.GREEN),
		FANG("item/gear/enchantedfang.png", Color.MAGENTA), TROPHY("item/gear/trophy.png", Color.GOLDENROD),

		BOOTS("item/gear/boots.png", Color.TAN), CLOAK("item/gear/cloak/cloak.png", Color.OLIVE),
		CLOAK_RED("item/gear/cloak/cloak_red.png", Color.RED),
		CLOAK_PURPLE("item/gear/cloak/cloak_purple.png", Color.PURPLE),
		CLOAK_BROWN("item/gear/cloak/cloak_brown.png", Color.BROWN), LINK("item/gear/magiclink.png", Color.SKY),
		...
		TORCH("item/gear/torch/torch.png", Color.ORANGE), TORCH_RED("item/gear/torch/torch_red.png", Color.RED),
		TORCH_BLUE("item/gear/torch/torch_blue.png", Color.ROYAL),
		TORCH_GREEN("item/gear/torch/torch_green.png", Color.CHARTREUSE), SCROLL("item/gear/scroll.png", Color.WHITE),
		ROCKET_BOOTS("item/gear/rocket_boots.png", Color.GRAY), MAGNET("item/gear/magnet/magnet.png", Color.RED),
		MAGNET_BLUE("item/gear/magnet/magnet_blue.png", Color.ROYAL),
		MAGNET_GRAY("item/gear/magnet/magnet_gray.png", Color.LIGHT_GRAY),
		MAGNET_BLACK("item/gear/magnet/magnet_black.png", Color.DARK_GRAY), SCOPE("item/gear/scope.png", Color.ROYAL),
		FURYBOOTS("item/gear/furyboots.png", Color.RED), DIRTYBOOTS("item/gear/dirtyboots.png", Color.BROWN),
		PROXIMITY_SUIT("item/gear/proximity_suit.png", Color.WHITE),
		BLOOD_AMULET("item/gear/blood_amulet.png", Color.FIREBRICK), POWERBALL("item/gear/powerball.png", Color.ORANGE),
		HOURGLASS("item/gear/hourglass.png", Color.CYAN), MEDICINE("item/gear/medicine.png", Color.MAROON),
		POISON_AMULET("item/gear/poison_amulet.png", Color.GREEN),
		WIRE_CUTTERS("item/gear/wire_cutters.png", Color.SCARLET), DIFFUSER("item/gear/diffuser.png", Color.ROYAL),
		ELECTRIC_AMULET("item/gear/electric_amulet.png", Color.YELLOW), CHEST_TOY("item/gear/chest.png", Color.TAN),
		FROZEN_RING("item/gear/frozen_ring.png", Color.CYAN), FIRE_AMULET("item/gear/fire_amulet.png", Color.RED),
		NULLZONE_RING("item/gear/nullzone_ring.png", Color.MAGENTA),
		VIRUS_VIAL("item/gear/virus_vial.png", Color.PURPLE), RADIALITH("item/gear/radialith.png", Color.RED),
		INFUSER("item/gear/infuser.png", new Color(0f, 1f, 179 / 255f, 1f)),

		BOOMERANG("item/weapon/boomerang.png", Color.ROYAL), FORK("item/weapon/special/fork.png", Color.LIGHT_GRAY),
	...
	public void draw(int x, int y, int originX, int originY, int width, int height, boolean inGUI, boolean spin,
			float a) {
		Graphics.begin();
		Graphics.setColor(Color.WHITE, !inGUI);
		if (!inGUI && !(this instanceof Heart && ((Heart) this).getHeartType() == Heart.Type.HEALTH_MINI)
				&& Gdx.graphics.getFramesPerSecond() > 50) {
			Graphics.draw(shadow, x - 25, y - 25);
		}
		float rotation = this == Realms.getGame().gui().getInfoItem() && Realms.getGame().getWindow() == Window.BACKPACK
				? (float) Math.cos((double) life / 20)
				: 1;
		if (this instanceof CombatGear && ((CombatGear) (this)).texture() != null) {
			((CombatGear) (this)).texture().draw(x, y, originX, originY, width, height, spin ? rotation : 1, 1, 0, a,
					!inGUI);
		} else {
			Graphics.setA(a);
	...
	public void drawCooldown(int x, int y, float a) {
		if ((this instanceof Weaponizable || this instanceof Activatable) && !(this instanceof Material)) {
			...
			if (((CombatGear) this).getCooldown() > 0 && cooldownMax > 0) {
				...
				Graphics.draw(new TextureRegion(Slot.Type.slots(), 250,
						50 - 50 * ((CombatGear) this).getCooldown() / cooldownMax, 50,
						50 * ((CombatGear) this).getCooldown() / cooldownMax), x, y);
				Graphics.resetColor();
				Graphics.end();
			}
		}
	...
	public void drawWindup(int x, int y, float a) {
		if (this instanceof WindupWeapon) {
			...
			if (windup > 0) {
				...
				Graphics.draw(new TextureRegion(Slot.Type.slots(), 250,
						50 - 50 * windup / ((WindupWeapon) this).getWindupMin(), 50,
						50 * windup / ((WindupWeapon) this).getWindupMin()), x, y);
				Graphics.resetColor();
				Graphics.end();
			}
		}
		if (this instanceof ShieldActive) {
			...
			if (meter > 0) {
				...
				Graphics.begin();
				Graphics.draw(
						new TextureRegion(Slot.Type.slots(), 250, 50 - 50 * meter / ((ShieldActive) this).getMeterMax(),
								50, 50 * meter / ((ShieldActive) this).getMeterMax()),
						x, y);
				Graphics.end();
				Graphics.resetColor();
			}
		...
		if (this instanceof Link) {
			...
			if (holdingTime > 0) {
				...
				if (((Link) this).isLinked()) {
					Graphics.draw(new TextureRegion(Slot.Type.slots(), 250, 50 - 50 * holdingTime / Link.HOLD_TIME_TP,
							50, 50 * holdingTime / Link.HOLD_TIME_TP), x, y);
				}
				Graphics.draw(new TextureRegion(Slot.Type.slots(), 250, 50 - 50 * holdingTime / Link.HOLD_TIME_SET, 50,
						50 * holdingTime / Link.HOLD_TIME_SET), x, y);
				Graphics.end();
				Graphics.resetColor();
			}
		}
	...
	public boolean equals(Object other) {
		...
		if (item != null && name.equals(item.name)) {
			if (this instanceof CombatGear && item instanceof CombatGear) {
				return ((CombatGear) this).equalsGear((CombatGear) item);
			} else {
				return true;
			}
		}
...
//	public boolean testHandicap() {
//		return Realms.getGame().getRoom().testHandicap(this);
//	}

	public boolean testHandicap(Check check) {
		return Realms.getGame().getRoom().testHandicap(this, check);
	}

	public static boolean equalType(Item item1, Item item2) {
		return item1 != null && item2 != null && (item1.getClass().equals(item2.getClass())
				|| (item1.getClass().isAnonymousClass() && item2.getClass().isAssignableFrom(item1.getClass()))
				|| (item2.getClass().isAnonymousClass() && item1.getClass().isAssignableFrom(item2.getClass())));
	}

	public boolean keyPressed() {
		return owner instanceof Player && (Gdx.input.isKeyPressed((int) Option.BIND_ATTACK_DOWN.button.getState())
				|| Gdx.input.isKeyPressed((int) Option.BIND_ATTACK_LEFT.button.getState())
				|| Gdx.input.isKeyPressed((int) Option.BIND_ATTACK_RIGHT.button.getState())
				|| Gdx.input.isKeyPressed((int) Option.BIND_ATTACK_UP.button.getState()));
	}
